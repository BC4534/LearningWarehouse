...
for i, line in enumerate(lines):
    ...
    if len(parts) == 2:
        ...
        for sheet_name in sheet_names:
            ...
            has_chinese_match = False
            original_value = value
            chinese_parts = re.findall(r'[\u4e00-\u9fa5]+', value)
            if chinese_parts:
                for chinese_part in chinese_parts:
                    processed_chinese = preprocess_text(chinese_part)
                    ...
                    if not chinese_match.empty:
                        ...
                        value = value.replace(chinese_part, chinese_translation)
                        has_chinese_match = True
                if has_chinese_match:
                    new_line = f"{key}={value}\n"
                    lines[i] = new_line
                    chinese_matched_lines.append(new_line)
                    is_matched = True
                    break
                else:
                    # 若中文部分没匹配上，恢复原词条值
...
# 将提取中文部分匹配上的词条写入新的 txt 文件
with open('户外柜国际化_提取中文匹配.txt', 'w', encoding='utf-8') as txt_file:
    txt_file.writelines(chinese_matched_lines)
